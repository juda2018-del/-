...
function detailView(storyId){
  ...
  ${episodes.map((ep,i)=>`<div class="episode"><button class="episode-index" data-play-episode="${story.id}|${ep.id}">${i+1}</button><div><h4>${esc(ep.title)}</h4><p>${esc(ep.duration)} · ${ep.free?'مجانية':'تفتح عند تفعيل الباقات'}${ep.audioSrc&&ep.audioSrc!==DEMO_AUDIO_SRC?' · صوت خاص':''}</p></div>${ep.free?`<button class="tiny-btn" data-play-episode="${story.id}|${ep.id}">تشغيل</button>`:`<span class="lock">قريباً</span>`}</div>`).join('') || `<div class="empty">ماكو حلقات بعد.</div>`}
...
function playerView(){
  ...
  return `<section class="player-page"><div class="player-stage ${state.player.playing?'':'paused'}">
    ...
    <div class="player-info"><small>${state.player.kind==='fm'?'جَزَل FM مباشر':esc(story?.genre||'جَزَل')}</small><h1>${esc(state.player.title||'جَزَل')}</h1><p>${esc(state.player.subtitle||'اسمع القصة للآخر')}</p></div>
